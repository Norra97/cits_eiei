...
router.post('/request', authenticateToken, isUser, async (req, res) => {
  ...
  console.log(`[BORROW REQUEST] User: ${Borrowname}, Assetid: ${Assetid}, Borrowdate: ${Borrowdate}, ReturnDate: ${ReturnDate}, Activity: ${Activity}, UsageType: ${UsageType}, Time: ${new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', hour12: false })}`);
  res.json({ id });
});
// Staff อนุมัติ
router.post('/approve/:id', authenticateToken, isStaff, async (req, res) => {
  ...
  console.log(`[APPROVE] Staff: ${req.user.username}, RequestId: ${req.params.id}, Time: ${new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', hour12: false })}`);
...
router.post('/reject/:id', authenticateToken, isStaff, async (req, res) => {
  ...
  console.log(`[REJECT] Staff: ${req.user.username}, RequestId: ${req.params.id}, Comment: ${Comment}, Time: ${new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', hour12: false })}`);
  res.json({ message: 'rejected' });
});
// User แจ้งคืน
router.post('/return/:id', authenticateToken, isUser, async (req, res) => {
  await BorrowRequest.returnItem(req.params.id);
  console.log(`[RETURN] User: ${req.user.username}, RequestId: ${req.params.id}, Time: ${new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', hour12: false })}`);
...
router.post('/confirm-return/:id', authenticateToken, isStaff, async (req, res) => {
  ...
  console.log(`[CONFIRM RETURN] Staff: ${req.user.username}, RequestId: ${req.params.id}, Time: ${new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', hour12: false })}`);
...
router.post('/reject-return/:id', authenticateToken, isStaff, async (req, res) => {
  ...
  console.log(`[REJECT RETURN] Staff: ${req.user.username}, RequestId: ${req.params.id}, Comment: ${Comment}, Time: ${new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', hour12: false })}`);
  res.json({ message: 'rejected' });
});
